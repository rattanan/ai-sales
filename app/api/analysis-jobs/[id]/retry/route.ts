...
import { retryAnalysisJob } from "@/server/services/analysis-job-service";
...
export async function POST(
  ...
  try {
    ...
    const result = await retryAnalysisJob(authorization, id);
    return Response.json(result, { status: result.ok ? 200 : 409 });
  } catch {
    return Response.json(
      failure("FORBIDDEN", "You cannot retry this analysis job."),
      { status: 403 },
    );
  }
}
