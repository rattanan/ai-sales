const loader = String.raw`(()=>{
  'use strict';
  if(window.AISalesWidget)return;
  const instances=new Map();
  const sizes={COMPACT:[320,480],STANDARD:[390,650],LARGE:[460,720]};
  function init(options){
    if(!options||!options.botId||!options.apiBase||!options.hostOrigin)throw new Error('AISalesWidget: botId, apiBase and hostOrigin are required');
    const apiBase=new URL(options.apiBase).origin;
    const hostOrigin=new URL(options.hostOrigin).origin;
    if(hostOrigin!==window.location.origin)throw new Error('AISalesWidget: hostOrigin must match the current page');
    ...
    function open(){frame.hidden=false;button.setAttribute('aria-expanded','true');button.setAttribute('aria-label','Close AI-Sales chat');send();frame.focus()}
    function close(){frame.hidden=true;button.setAttribute('aria-expanded','false');button.setAttribute('aria-label','Open AI-Sales chat');button.focus()}
  ...
  window.AISalesWidget={init};
  window.InsightKMWidget=window.AISalesWidget;
})();`;
