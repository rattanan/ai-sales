...
function page() {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI-Sales Widget Sample Host</title><style>body{margin:0;min-height:100vh;font-family:system-ui;background:linear-gradient(135deg,#eef2ff,#f8fafc);color:#172033}.content{max-width:760px;margin:auto;padding:72px 24px}h1{font-size:clamp(2rem,7vw,4rem);margin:0}.card{margin-top:28px;padding:24px;border:1px solid #dbe2f0;border-radius:20px;background:#ffffffcc;box-shadow:0 18px 50px #17203312}code{word-break:break-all}</style></head><body><main class="content"><p>Sample host application</p><h1>AI-Sales embedded securely.</h1><section class="card"><h2>Signed on the host server</h2><p>The browser receives a short-lived payload and signature, but never the signing secret.</p><p>Host origin: <code>${hostOrigin}</code></p></section></main><script src="${insightKmBase}/widget/v1.js"></script><script>AISalesWidget.init(${config});</script></body></html>`;
}
